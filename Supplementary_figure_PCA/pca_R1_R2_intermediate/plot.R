tiff("pca34_R1.tiff", width = 10, height = 9, units = 'in', res = 200)
C <- read.table(file="R1_34.beagle.gz.cov.cov")
m<-as.matrix(C)
e <- eigen(m, symm=TRUE)
...
tiff("pca34_R2.tiff", width = 10, height = 9, units = 'in', res = 200)
C <- read.table(file="R2_34.beagle.gz.cov.cov")
m<-as.matrix(C)
e <- eigen(m, symm=TRUE)
e$val <- e$val/sum(e$val)
cat(signif(e$val, digits=3)*100,"\n")
...
tiff("pca34_R1_R2_intermediate.tiff", width = 10, height = 9, units = 'in', res = 200)
C <- read.table(file="R1_R2_intermediate_34.beagle.gz.cov.cov")
m<-as.matrix(C)
e <- eigen(m, symm=TRUE)
e$val <- e$val/sum(e$val)
cat(signif(e$val, digits=3)*100,"\n")
