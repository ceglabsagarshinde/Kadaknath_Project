## Isolation by distance(IBD) calculation using Geographical cordinates of 5 population and pairwise mean Fst from popgenwindow.
library(sp)
library(reshape)
library("ecodist")
someCoords <- data.frame(long=c(111,103,99,127,77), lat=c(32,29,27,35,23))
## Geographical cordinates of 5 population in respective order (XBBC,LCEM,TBTC,YOSK,JETB)
apply(someCoords, 1, function(eachPoint) spDistsN1(as.matrix(someCoords), eachPoint, longlat=TRUE))->X
X[upper.tri(X)]->G
pop1<-c("XBBC","XBBC","LCEM","XBBC","LCEM","TBTC","XBBC","LCEM","TBTC","YOSK")
pop2<-c("LCEM","TBTC","TBTC","YOSK","YOSK","YOSK","JETB","JETB","JETB","JETB")
data.frame(pop1,pop2,G)->Geodist

Geodist2 <- with(Geodist, G)
nams <- with(Geodist, unique(c(as.character(pop1), as.character(pop2))))
class(Geodist2) <- "dist"
read.table(file="5pop_popgenwindow.fst",header=TRUE,sep="\t")->M
na.omit(melt(M))->N
...
tiff("5pop_popgenwindow.tif",width = 10, height = 9, units = 'in', res = 300)
...
## Isolation by distance(IBD) calculation using Geographical cordinates of 5 population and pairwise mean Fst from ANGSD.
library(sp)
library(reshape)
library("ecodist")
someCoords <- data.frame(long=c(111,103,99,127,77), lat=c(32,29,27,35,23))
...
nams <- with(Geodist, unique(c(as.character(pop1), as.character(pop2))))
class(Geodist2) <- "dist"
read.table(file="5pop_angsd.fst",header=TRUE,sep="\t")->M
na.omit(melt(M))->N
data.frame(pop1,pop2,N$value)->Gendist
Gendist2 <- with(Gendist, N$value)
nams <- with(Gendist, unique(c(as.character(pop1), as.character(pop2))))
...
tiff("5pop_angsd.tif",width = 10, height = 9, units = 'in', res = 300)
plot(G,N$value/(1-N$value),xlab=expression(bold("Distance in Km")),ylab=expression(paste(bold("F"[ST]*"/(1-F"[ST]*") from Autosomes"))),main="Isolation by Distance Pattern",col="blue",xlim=c(-100,6000),cex=1,pch=16)
text(G,N$value/(1-N$value),labels=paste(pop1,"  ",pop2),cex=0.7)
lm(N$value/(1-N$value)~G)->a
abline(a)
