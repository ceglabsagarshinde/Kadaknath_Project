...
tiff("KADK_CHIN_WHLL_RJF_BABA_50KB_chr11_main_figure.tif",res=100,width=5000,height=3000)
...
legend(6500000, 1,legend=c("KADK vs. CHIN","CHIN  vs. WHLL","KADK vs. WHLL"),col=c("black","purple","green"),lty=1,cex=5,lwd=15,bty = "n")
...
abline(v=300000,col="red",lty = 2,lwd=2)
abline(v=200000,col="red",lty = 2,lwd=2)
legend(6500000, 0.7,legend=c("P1=KADK P2=CHIN P3=WHLL"),lty=1,cex=5,lwd=15,bty = "n")
box(lwd=10)
...
tiff("KADK_CHIN_WHLL_RJF_BABA_50KB_chr_4_GABAR2_main_figure.tif",res=100,width=5000,height=3000)
par(mar = c(6,20,5,2),mgp=c(10,3,0.3))
par(mfrow=c(3,1))
scaffolds<-c(4)
for (scaff in scaffolds){
...
tiff("KADK_CHIN_WHLL_RJF_BABA_50KB_chr_4_C40RF33_main_figure.tif",res=100,width=5000,height=3000)
par(mar = c(6,20,5,2),mgp=c(10,3,0.3))
par(mfrow=c(3,1))
scaffolds<-c(4)
for (scaff in scaffolds){
...
read.table("KADK_WHLL.50kb.csv",header=T)->D
read.table("CHIN_WHLL.50kb.csv",header=T)->E
read.table("KADK_CHIN.50kb.csv",header=T)->F


pdf("WHLL.pdf",,width=500,height=100)
...
legend("topleft",legend=c("P1=KADK P2=CHIN P3=WHLL"),col=c("black"),lty=1,cex=5,lwd=15,bty = "n")
abline(h=0,col="red",lty = 2,lwd=5)
box(lwd=10)
...
D1=c(max(D$dxy_KADK_WHLL[D$scaffold==scaff],na.rm=T),max(E$dxy_CHIN_WHLL[E$scaffold==scaff],na.rm=T),max(F$Fst_KADK_CHIN[F$scaffold==scaff],na.rm=T))
D2=c(min(D$dxy_KADK_WHLL[D$scaffold==scaff],na.rm=T),min(E$dxy_CHIN_WHLL[E$scaffold==scaff],na.rm=T),min(F$Fst_KADK_CHIN[F$scaffold==scaff],na.rm=T))

par(mar = c(15,20,5,2),mgp=c(10,3,0.3))
plot(D$start[D$scaffold==scaff],D$dxy_KADK_WHLL[D$scaffold==scaff],ylab=expression(paste(bold("D"[XY]))),col="green",cex=2,cex.axis = 5, cex.main=6,cex.lab=4,cex.main=7,type="l",lwd=15,ylim=c(min(D2),max(D1)),xlab=paste("position along chromosome ", scaff ,sep=""),cex.lab=6)
j=dim(dd2)[1]
if (j==0){
print(j)
} else {
...
lines(E$start[E$scaffold==scaff],E$dxy_CHIN_WHLL[E$scaffold==scaff],col="purple",cex=2,cex.axis = 5, cex.main=6,cex.lab=6,cex.main=7,lwd=15)
